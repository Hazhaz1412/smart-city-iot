...
def get_openaq_api_key(user=None):
    """Get OpenAQ API key from database or settings"""
    try:
        from .models import get_api_key_for_provider
        key = get_api_key_for_provider('openaq', user)
        if key:
            return key
    ...
    return getattr(settings, 'OPENAQ_API_KEY', None)
...
class OpenAQClient:
    ...
    def __init__(self, api_key: str = None, user=None):
        self.api_key = api_key or get_openaq_api_key(user)
